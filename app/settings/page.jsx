    ...
    <div style={{display:'flex',minHeight:'100vh',background:theme.bg,color:theme.color,fontFamily:'system-ui,-apple-system,sans-serif',position:'relative'}}>
      ...
      <div style={{marginLeft:isMobile?0:'240px',flex:1,padding:isMobile?'20px 16px 100px':'32px',position:'relative',zIndex:1}}>
        ...
        <div style={{marginBottom:'24px'}}>
          ...
          <p style={{color:theme.textMuted,margin:0,fontSize:'14px'}}>Créez une carte unique pour votre commerce</p>
        </div>
        ...
        <div style={{display:'grid',gridTemplateColumns:isMobile?'1fr':'1fr 1fr',gap:'20px'}}>
          <div>
            <div style={{display:'flex',gap:'6px',marginBottom:'16px',flexWrap:'wrap'}}>
              {tabs.map(tab => (
                <button key={tab.id} onClick={() => setActiveTab(tab.id)} style={{background:activeTab===tab.id?'#d4af37':theme.cardBg,color:activeTab===tab.id?'white':theme.textMuted,border:'none',borderRadius:'8px',padding:'8px 14px',cursor:'pointer',fontSize:'12px',fontWeight:'600',transition:'all 0.2s'}}>
                  {tab.label}
                </button>
              ))}
            </div>

            <div style={{background:theme.cardBg,border:`1px solid ${theme.cardBorder}`,borderRadius:'16px',padding:'20px',backdropFilter:'blur(10px)',minHeight:'300px'}}>

              {activeTab === 'programme' && (
                <div>
                  <h3 style={{margin:'0 0 16px',fontSize:'15px',fontWeight:'600',color:theme.textSecondary}}>🎯 Type de programme</h3>
                  <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:'10px',marginBottom:'20px'}}>
                    <div onClick={() => setForm({...form, loyalty_type: 'stamps'})} style={{background:form.loyalty_type==='stamps'?'rgba(212,175,55,0.15)':theme.cardBg,border:form.loyalty_type==='stamps'?'2px solid #d4af37':`1px solid ${theme.cardBorder}`,borderRadius:'12px',padding:'16px',cursor:'pointer',textAlign:'center'}}>
                      <div style={{fontSize:'28px',marginBottom:'8px'}}>🎫</div>
                      <p style={{fontWeight:'600',margin:'0 0 4px',fontSize:'14px'}}>Tampons</p>
                      <p style={{fontSize:'11px',color:theme.textMuted,margin:0}}>Ex: 10 tampons = 1 café offert</p>
                    </div>
                    <div onClick={() => setForm({...form, loyalty_type: 'points'})} style={{background:form.loyalty_type==='points'?'rgba(212,175,55,0.15)':theme.cardBg,border:form.loyalty_type==='points'?'2px solid #d4af37':`1px solid ${theme.cardBorder}`,borderRadius:'12px',padding:'16px',cursor:'pointer',textAlign:'center'}}>
                      <div style={{fontSize:'28px',marginBottom:'8px'}}>⭐</div>
                      <p style={{fontWeight:'600',margin:'0 0 4px',fontSize:'14px'}}>Points</p>
                      <p style={{fontSize:'11px',color:theme.textMuted,margin:0}}>Ex: 1€ = 1 point</p>
                    </div>
                  </div>

                  {form.loyalty_type === 'stamps' && (
                    <div>
                      <label style={{fontSize:'12px',color:theme.textMuted,fontWeight:'600',textTransform:'uppercase',letterSpacing:'0.8px',display:'block',marginBottom:'8px'}}>Tampons pour récompense</label>
                      <div style={{display:'flex',gap:'8px',flexWrap:'wrap'}}>
                        {[5,8,10,12,15,20].map(n => (
                          <button key={n} onClick={() => setForm({...form,card_stamps_required:n})} style={{background:form.card_stamps_required===n?'#d4af37':theme.cardBg,color:form.card_stamps_required===n?'white':theme.textMuted,border:form.card_stamps_required===n?'none':`1px solid ${theme.cardBorder}`,borderRadius:'8px',padding:'8px 16px',cursor:'pointer',fontSize:'14px',fontWeight:'600'}}>
                            {n}
                          </button>
                        ))}
                      </div>
                      <p style={{fontSize:'12px',color:theme.textMuted,margin:'8px 0 0'}}>{form.card_stamps_required} tampons = 1 récompense</p>
                    </div>
                  )}
                  ...
                  {form.loyalty_type === 'points' && (
                    <div>
                      <div style={{marginBottom:'16px'}}>
                        <label style={{fontSize:'12px',color:theme.textMuted,fontWeight:'600',textTransform:'uppercase',letterSpacing:'0.8px',display:'block',marginBottom:'8px'}}>Points par euro dépensé</label>
                        <div style={{display:'flex',gap:'8px',flexWrap:'wrap'}}>
                          {[1,2,5,10].map(n => (
                            <button key={n} onClick={() => setForm({...form,points_per_euro:n})} style={{background:form.points_per_euro===n?'#d4af37':theme.cardBg,color:form.points_per_euro===n?'white':theme.textMuted,border:form.points_per_euro===n?'none':`1px solid ${theme.cardBorder}`,borderRadius:'8px',padding:'8px 16px',cursor:'pointer',fontSize:'14px',fontWeight:'600'}}>
                              {n} pt/€
                            </button>
                          ))}
                        </div>
                      ...
                      <div>
                        <label style={{fontSize:'12px',color:theme.textMuted,fontWeight:'600',textTransform:'uppercase',letterSpacing:'0.8px',display:'block',marginBottom:'8px'}}>Récompenses</label>
                        {form.rewards.map((reward, i) => (
                          <div key={i} style={{display:'flex',gap:'8px',marginBottom:'8px',alignItems:'center'}}>
                            <input type="number" value={reward.points} onChange={e => { const r=[...form.rewards]; r[i]={...r[i],points:parseInt(e.target.value)}; setForm({...form,rewards:r}); }} placeholder="Points" style={{width:'80px',background:theme.inputBg,border:`1px solid ${theme.inputBorder}`,borderRadius:'8px',padding:'8px',color:theme.color,fontSize:'13px',outline:'none'}}/>
                            <span style={{color:theme.textMuted,fontSize:'13px'}}>pts =</span>
                            <input value={reward.reward} onChange={e => { const r=[...form.rewards]; r[i]={...r[i],reward:e.target.value}; setForm({...form,rewards:r}); }} placeholder="Ex: Café offert" style={{flex:1,background:theme.inputBg,border:`1px solid ${theme.inputBorder}`,borderRadius:'8px',padding:'8px',color:theme.color,fontSize:'13px',outline:'none'}}/>
              ...
              {activeTab === 'couleur' && (
                <div>
                  <h3 style={{margin:'0 0 16px',fontSize:'15px',fontWeight:'600',color:theme.textSecondary}}>🎨 Couleur de fond</h3>
                  <div style={{display:'grid',gridTemplateColumns:'repeat(4,1fr)',gap:'10px'}}>
                    {COLORS.map(c => (
                      <div key={c.value} onClick={() => setForm({...form,card_color:c.value,card_background_style:'solid'})} style={{cursor:'pointer'}}>
                        ...
                        <p style={{fontSize:'10px',color:theme.textMuted,textAlign:'center',margin:'4px 0 0'}}>{c.label}</p>
                      </div>
                    ))}
                  </div>
                </div>
              ...
              {activeTab === 'police' && (
                <div>
                  <h3 style={{margin:'0 0 16px',fontSize:'15px',fontWeight:'600',color:theme.textSecondary}}>✍️ Police d'écriture</h3>
                  <div style={{display:'flex',flexDirection:'column',gap:'10px'}}>
                    {FONTS.map(f => (
                      <div key={f.value} onClick={() => setForm({...form,card_font:f.value})} style={{background:form.card_font===f.value?'rgba(212,175,55,0.1)':theme.cardBg,border:form.card_font===f.value?'1px solid rgba(212,175,55,0.4)':`1px solid ${theme.cardBorder}`,borderRadius:'10px',padding:'14px 16px',cursor:'pointer',transition:'all 0.2s'}}>
                        <p style={{fontFamily:f.value,fontSize:'16px',margin:'0 0 4px',color:theme.color}}>{f.label}</p>
                        <p style={{fontFamily:f.value,fontSize:'12px',margin:0,color:theme.textMuted}}>Café de la Plage · 7/10 tampons</p>
                      </div>
                    ))}
                  </div>
                </div>
              ...
              {activeTab === 'general' && (
                <div>
                  <h3 style={{margin:'0 0 16px',fontSize:'15px',fontWeight:'600',color:theme.textSecondary}}>⚙️ Paramètres généraux</h3>
                  <div style={{marginBottom:'16px'}}>
                    <label style={{fontSize:'12px',color:theme.textMuted,fontWeight:'600',textTransform:'uppercase',letterSpacing:'0.8px',display:'block',marginBottom:'8px'}}>Nom affiché sur la carte</label>
                    <input value={form.card_logo_text} onChange={e => setForm({...form,card_logo_text:e.target.value})} placeholder="Nom de votre commerce" style={{width:'100%',background:theme.inputBg,border:`1px solid ${theme.inputBorder}`,borderRadius:'8px',padding:'10px 12px',color:theme.color,fontSize:'14px',boxSizing:'border-box',outline:'none'}} onFocus={e=>e.target.style.borderColor='rgba(212,175,55,0.5)'} onBlur={e=>e.target.style.borderColor=theme.inputBorder}/>
                  </div>
                </div>
              )}
            </div>
          ...
          <div style={{background:theme.cardBg,border:`1px solid ${theme.cardBorder}`,borderRadius:'16px',padding:'24px',backdropFilter:'blur(10px)',display:'flex',flexDirection:'column',alignItems:'center',justifyContent:'center',position:isMobile?'static':'sticky',top:'32px',alignSelf:'flex-start'}}>
            <h3 style={{margin:'0 0 24px',fontSize:'15px',fontWeight:'600',alignSelf:'flex-start',color:theme.textSecondary}}>👁️ Aperçu en temps réel</h3>
            ...
            <p style={{fontSize:'11px',color:theme.textMuted,marginTop:'16px',textAlign:'center'}}>L'aperçu se met à jour en temps réel ✨</p>
          </div>
        </div>
      </div>
