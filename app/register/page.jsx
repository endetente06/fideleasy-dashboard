  ...
  if (success) return (
    <div style={{minHeight:'100vh',background:'#07070f',color:'white',fontFamily:'sans-serif',display:'flex',alignItems:'center',justifyContent:'center'}}>
      <div style={{textAlign:'center'}}>
        ...
        <h2 style={{fontSize:'24px',fontWeight:'bold',marginBottom:'8px'}}>Compte créé !</h2>
        <p style={{color:'#6060a0'}}>Redirection vers la connexion...</p>
      </div>
    </div>
  );
    ...
    <div style={{minHeight:'100vh',background:'#07070f',color:'white',fontFamily:'sans-serif',display:'flex',alignItems:'center',justifyContent:'center',padding:'24px'}}>
      <div style={{width:'100%',maxWidth:'400px'}}>
        ...
        <div style={{background:'#0f0f1c',border:'1px solid #1e1e35',borderRadius:'16px',padding:'32px'}}>
          ...
          <div style={{fontSize:'11px',color:'#6060a0',marginBottom:'6px',textTransform:'uppercase',letterSpacing:'0.8px'}}>Nom du commerce</div>
          <input placeholder="Ex: Café de la Plage" value={form.shop_name} onChange={e=>setForm({...form,shop_name:e.target.value})}
          ...
          <button onClick={register} disabled={loading || !form.email || !form.password || !form.shop_name}
          ...
          <div style={{textAlign:'center',marginTop:'16px',fontSize:'13px',color:'#6060a0'}}>
            Déjà un compte ? <span style={{color:'#4f6ef7',cursor:'pointer'}} onClick={()=>window.location.href='/login'}>Se connecter</span>
          </div>
        </div>
      </div>
    </div>
